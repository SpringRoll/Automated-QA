...
const findEslintIgnore = (startingPath) => {
  let eslintIgnorePath = path.join(expandPath(startingPath), '.eslintignore');
  ...
  while (true) {
    ...
    // If we're in the root directory, we can't go up any more. Break out
    const currentDirectory = path.dirname(eslintIgnorePath);
    if (currentDirectory === '/') {
      return null;
    }
    ...
    eslintIgnorePath = path.join(parentDirectory, '.eslintignore');
  }
};
